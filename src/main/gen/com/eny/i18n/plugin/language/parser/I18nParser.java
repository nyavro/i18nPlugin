...
public class I18nParser implements PsiParser, LightPsiParser {
  ...
  private static boolean compositeKey_1_0(PsiBuilder b, int l) {
    ...
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, KEY_SEPARATOR);
    r = r && keyItem(b, l + 1);
  ...
  /* ********************************************************** */
  // KEY
  public static boolean keyItem(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "keyItem")) return false;
    if (!nextTokenIs(b, KEY)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, KEY);
    exit_section_(b, m, KEY_ITEM, r);
    return r;
  }
